...
metadata:
  short-description: Run an AI-native software development lifecycle
---

# AI-Native SDLC

Reframe the software lifecycle around the fact that **code is no longer the bottleneck**.
The agent handles volume (writing intent, specs, tests, code, reviews); the human stays
...
4. **Institutional knowledge → `AGENTS.md`.** Keep durable project conventions there
   (Codex's equivalent of a versioned `CLAUDE.md`), not in one-off prompts.
...
in `AGENTS.md`. Commit code referencing the spec.
...
```bash
SKILL_DIR="${CODEX_HOME:-$HOME/.codex}/skills/ai-native-sdlc/templates"
cp "$SKILL_DIR/intent.md" ./intent.md   # Plan
cp "$SKILL_DIR/spec.md"   ./spec.md     # Design
cp "$SKILL_DIR/AGENTS.md" ./AGENTS.md   # Build — durable conventions (Codex auto-loads this)
```

Each template is Codex-flavored: `intent.md` names the **policy skills** and **sandbox/access
mode** up front; `spec.md` ties requirements → tests/evals and records the **approval mode +
`codex exec`** deploy path; `AGENTS.md` is the repo's auto-loaded agent guide.
